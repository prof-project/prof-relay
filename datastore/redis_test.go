...
func TestSavingProf(t *testing.T) {

	/* Create the deneb.executionpayload from the yaml file */
	...
	/* Finish creating the deneb.executionpayload from the yaml file */

	// Setup redis instance
	cache := setupTestRedis(t)
	slot := executionPayload.BlockNumber // dummy doesnt matter
	proposerpk := "dummy"
	blockHashString := strings.ToLower(c["block_hash"].(string))
	...
	err = cache.SavePayloadContentsProf(slot, proposerpk, blockHashString, executionPayloadAndBlobsBundle)
	require.NoError(t, err)
	// check if the payload was saved
	resp, err := cache.GetPayloadContentsDeneb(slot, proposerpk, blockHashString)
	require.NoError(t, err)
	// check that the resp is correct
	require.Equal(t, resp.Deneb.ExecutionPayload, executionPayload)
